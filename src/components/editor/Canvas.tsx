...
export default function Canvas({ wallLength, wallHeight, currentTool, onElementsChange }: CanvasProps) {
  ...
  const drawElement = (ctx: CanvasRenderingContext2D, element: CanvasElement) => {
    ...
    switch (element.type) {
      ...
      case "circle":
        ctx.beginPath();
        ctx.arc(element.points[0].x, element.points[0].y, 15, 0, 2 * Math.PI);
        ctx.fill();
        ctx.stroke();
        break;
      case "double-circle":
        ctx.beginPath();
        ctx.arc(element.points[0].x - 10, element.points[0].y, 10, 0, 2 * Math.PI);
        ctx.fill();
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(element.points[0].x + 10, element.points[0].y, 10, 0, 2 * Math.PI);
        ...
        break;
      case "square":
        ctx.fillRect(element.points[0].x - 10, element.points[0].y - 10, 20, 20);
        ctx.strokeRect(element.points[0].x - 10, element.points[0].y - 10, 20, 20);
        break;
      case "large-square":
        ctx.fillRect(element.points[0].x - 15, element.points[0].y - 15, 30, 30);
        ctx.strokeRect(element.points[0].x - 15, element.points[0].y - 15, 30, 30);
        break;
    }
  };
  ...
  const handleMouseUp = () => {
    ...
    const newElement: CanvasElement = {
      ...
      length: currentTool.type === "area" ? undefined : length,
      area: currentTool.type === "area" ? calculateArea(currentPoints) : undefined,
    };
  ...
  const calculateArea = (points: { x: number; y: number }[]): number => {
    ...
    return area / (SCALE * SCALE); // Convert pixels² to meters²
  };
    ...
    <Card className="p-4">
      <div className="flex justify-between items-center mb-4">
        <div className="text-sm text-muted-foreground">
          Размер стены: {wallLength}м × {wallHeight}м
        </div>
        <button
          onClick={handleClear}
